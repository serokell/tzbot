...
>>> parseWithEmptyContext "How about Wednesday at 10:00 / 11:00 or 14:00 / 15:00"
...
applyContexts :: [ApplyContextToken] -> State TimeContext [TimeReference]
applyContexts lst = mapMaybe getTimeRef <$> mapM processACTToken lst
  ...
  processACTToken = \case
    d@(ACTDateReference ma) -> do
      setCurrentDate $ mtValue ma
      pure d
    ACTTimeReference tr -> do
      tr' <- useCurrentDate tr >>= useCurrentLoc
      pure $ ACTTimeReference tr'
    where
      setCurrentDate :: DateReference -> State TimeContext ()
      setCurrentDate ref = tcCurrentDateRefL .= Just ref

      useCurrentDate :: TimeReference -> State TimeContext TimeReference
      useCurrentDate tr = case trDateRef tr of
        Nothing -> do
          defaultCurDate <- tcCurrentDateRef <$> get
          pure $ tr {trDateRef = defaultCurDate}
        Just dr -> do
          setCurrentDate dr
          pure tr

      setCurrentLoc :: LocationReference -> State TimeContext ()
      setCurrentLoc ref = tcCurrentLocRefL .= Just ref

      useCurrentLoc :: TimeReference -> State TimeContext TimeReference
      useCurrentLoc tr = case trLocationRef tr of
        Nothing -> do
          defaultCurLoc <- tcCurrentLocRef <$> get
          pure $ tr {trLocationRef = defaultCurLoc}
        Just lr -> do
          setCurrentLoc lr
          pure tr
...
applyContextTokenParser = choice'
  ...
  , L.singleton . ACTDateReference . uncurry Matched <$> match dateRefParser
  ]
